...
9. **명리 용어 즉시 해설(필수)**: 일간/일주/원국/월주처럼 독자가 모를 수 있는 용어가 처음 나오면,
   바로 뒤에 아주 짧은 해설 한 줄을 붙인다(예: "내 일간은 태어난 날의 첫 글자예요", "원국은 타고난
   여덟 글자예요"). 2026-09-22 벤치마크 실측(@taebaek_saju): 이 한 줄이 있어서 명리를 모르는
   독자도 끝까지 읽는다. 설명을 길게 늘리지 말고 한 줄로 끝낸다.
10. **CTA 직전 "정보 갭"(필수)**: CTA 문장 바로 앞에, 이 글만으로는 답을 낼 수 없는 이유를 2~3줄로
   구체적으로 밝힌다 - 무엇이 더 필요한지를 명시해야 한다(예: "띠로는 여기까지밖에 안 나옵니다 /
   배우자 자리는 태어난 날에서, 자녀 자리는 태어난 시간에서 나옵니다 / 여덟 글자와 대운을 같이
   봐야 시기가 나옵니다"). 2026-09-22 벤치마크 실측(@taebaek_saju)에서 이 구조가 매 글 마지막에
   빠짐없이 들어가 상담이 필요한 이유를 납득시킨다. "정확한 건 원국을 봐야 한다" 같은 막연한 한 줄로
   대체하지 말고, 태어난 날·시간·대운 중 무엇이 왜 필요한지를 소재에 맞게 쓴다.
