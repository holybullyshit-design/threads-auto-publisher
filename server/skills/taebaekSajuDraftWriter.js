...
// 2026-09-22 사용자 지시: 벤치마크 채널(@taebaek_saju 등)의 "구조/기획"은 배우되, 그 채널
// 특유의 문장을 그대로 쓰면 표절 시비가 붙는다. 명리학 판별법 같은 사실이나 파트 구성 같은
// 형식은 누구의 것도 아니지만, 아래 문장들은 그 채널의 고유 표현이므로 생성물에 그대로
// 나오면 실패로 처리해서 다시 쓰게 한다(부분 일치도 차단).
const BANNED_BENCHMARK_PHRASES = [
  "저를 찾아오십시오",
  "저를 찾아오세요",
  "내 일간은 태어난 날의 첫 글자",
  "일간만으로는 시기까지",
...
const CLOSING_STYLES = [
  ...
  {
    // 2026-09-22 벤치마크 실측(@taebaek_saju "앞으로 좋은 남자만 만나게 되는 일간", 조회 2.1천):
    // 마지막 파트가 팁 3개 나열이 아니라 "정유월 안에 하나만 하세요 / 먼저 연락하는 걸 멈추는
    // 겁니다"처럼 이번 달에 할 단 하나의 행동으로 좁혀진다 - 실행 부담이 낮아 저장·실천 유인이 크다.
    id: "singleAction",
    instruction: `단일 행동 처방형: 마지막 파트에서 팁을 여러 개 나열하지 말고, 이번 달 안에 할 수 있는
    행동 "딱 하나"만 지목한다("이번 달 안에 하나만 하세요" 류의 한 줄 뒤에 그 행동 한 개). 그 행동은
    추상적 조언이 아니라 당장 멈추거나 시작할 수 있는 구체적 동작이어야 한다(예: "먼저 연락하는 걸
    멈추세요", "미뤄둔 연락 하나만 정리하세요"). 그리고 그 행동이 왜 이 달에 효과가 있는지 한 줄로
    덧붙인 뒤 CTA로 넘어간다.`,
  },
  {
...
9. **명리 용어 즉시 해설(필수, 표현은 매번 새로)**: 일간/일주/원국/월주처럼 독자가 모를 수 있는
   용어가 처음 나오면 아주 짧게 풀어준다. 단, 정해진 한 문장을 반복하지 말고 그 글의 흐름에 맞게
   매번 다르게 쓴다 - 예를 들어 같은 "일간"도 어떤 글에서는 "태어난 날이 어떤 글자로 시작하는지",
   어떤 글에서는 "만세력에서 생일 칸 맨 위에 뜨는 글자", 어떤 글에서는 "사주 여덟 글자 중 나를
   가리키는 한 글자"처럼 다른 각도로 설명한다. 용어 자체를 괄호로 짧게 옮겨쓰는 방식(축=소띠 글자,
   미=양띠 글자)도 섞어 쓴다. 설명은 한 줄로 끝내고, 아래 [절대 금지 - 표현 베끼기] 목록의 문장은
   쓰지 않는다.
10. **CTA 직전 "정보 갭"(필수, 표현은 매번 새로)**: CTA 바로 앞에, 이 글만으로는 답이 안 나오는
   이유를 2~3줄로 구체적으로 쓴다. 핵심은 "무엇이 더 있어야 하는지"를 소재에 맞게 특정하는 것이다
...
않는다): "저를 찾아오십시오", "내 일간은 태어난 날의 첫 글자", "일간만으로는 시기까지 안 나옵니다",
"OO 깐 사람", "모든 게 술술 풀리는", "복채", "선착순 예약", "심층적 고급 사주", "장인의 통찰".
명리학 판별법·오행 원리 같은 사실과, 파트를 나눠 이어가는 형식 자체는 자유롭게 쓴다 - 금지되는 건
남의 문장과 말버릇을 그대로 옮기는 것뿐이다.
...
async function writeThreadDraft({
  ...
  const banned = parts.map((p) => findBannedPhrase(p)).find(Boolean);
  if (banned) {
    throw new Error(`벤치마크 채널 고유 표현이 그대로 들어갔습니다("${banned}") - 우리 표현으로 다시 씁니다.`);
  }
