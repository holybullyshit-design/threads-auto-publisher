...
// 지어낸 숫자는 없다 - 전부 캐시(data/insights-cache.json)에서 읽거나, 캐시가 아예 없으면
// 그 자리에서 한 번 실제로 가져온다. 실시간 API 호출은 refreshAccount 안에서 시간대별로
// 캐시가 오래됐을 때만 일어난다(threadsInsights.js 참고).
...
app.listen(PORT, () => {
  ...
  setTimeout(() => {
    threadsInsights.refreshAll({}).catch((err) => console.error("[insights] 초기 갱신 실패:", err.message));
  }, 5000);
  setInterval(() => {
    threadsInsights.refreshAll({}).catch((err) => console.error("[insights] 주기 갱신 실패:", err.message));
  }, 60 * 60 * 1000);
});
