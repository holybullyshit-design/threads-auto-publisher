...
const threadsInsights = require("./lib/threadsInsights");
const insightsSummary = require("./lib/insightsSummary");
...
// ---------- 관제탑 · 성과 분석 (실제 Threads Insights API) ----------
// 계정별 조회수/좋아요/답글/팔로워와, 그 숫자에서 뽑은 "노출을 늘리려면" 추천을 만든다.
// 실제 집계 로직은 server/lib/insightsSummary.js에 있다 - claude.ai 모바일 아티팩트
// (tools/ops-dashboard/generate.js)도 같은 로직을 써서 숫자/추천 문구가 갈라지지 않게 하려고
// 공용 모듈로 뽑아뒀다.
const { buildInsightsSummary } = insightsSummary;

// 모바일 아티팩트는 클라우드 루틴이 fresh git clone에서 generate.js를 돌리기 때문에
// data/accounts.json(토큰)에 접근할 수 없다 - 그래서 매 갱신 후 "숫자만" 담은 요약을
// git에 커밋되는 tools/ops-dashboard/insights-summary.json으로 내보낸다(토큰 없음, 안전).
// GitHub 설정이 안 돼있거나 오프라인이어도 로컬 관제탑 탭은 캐시로 계속 동작해야 하므로
// 여기 실패는 조용히 로그만 남기고 삼킨다.
async function publishInsightsSummaryToGithub() {
  try {
    const accounts = accountsStore.listAccounts({ includeSecrets: true }).filter((a) => a.threadsUserId && a.accessToken);
    if (accounts.length === 0) return;
    const posts = await scheduleStore.listSchedule();
    const cache = threadsInsights.getCache();
    const summary = buildInsightsSummary(cache, accounts, posts);
    await githubStore.writeJsonFile(
      "tools/ops-dashboard/insights-summary.json",
      summary,
      "chore: 관제탑 성과 분석 데이터 갱신 [skip ci]"
    );
  } catch (err) {
    console.error("[insights] GitHub 요약 내보내기 실패:", err.message);
  }
}

app.get("/api/ops/insights", async (req, res) => {
  try {
    const accounts = accountsStore.listAccounts({ includeSecrets: true }).filter((a) => a.threadsUserId && a.accessToken);
    let cache = threadsInsights.getCache();
    if (Object.keys(cache).length === 0 && accounts.length > 0) {
      await threadsInsights.refreshAll({});
      cache = threadsInsights.getCache();
    }
    const posts = await scheduleStore.listSchedule();
    res.json(buildInsightsSummary(cache, accounts, posts));
  } catch (err) {
    handleError(res, err);
  }
});

app.post("/api/ops/insights/refresh", async (req, res) => {
  try {
    await threadsInsights.refreshAll({ force: Boolean(req.body?.force) });
    await publishInsightsSummaryToGithub();
    const accounts = accountsStore.listAccounts({ includeSecrets: true }).filter((a) => a.threadsUserId && a.accessToken);
    const posts = await scheduleStore.listSchedule();
    const cache = threadsInsights.getCache();
    res.json(buildInsightsSummary(cache, accounts, posts));
...
app.listen(PORT, () => {
  ...
  // 관제탑 "성과 분석"이 사용자가 클릭 안 해도 항상 최신에 가깝게 보이도록,
  // 서버 뜬 직후 한 번 + 이후 1시간마다 백그라운드로 Threads Insights를 갱신한다.
  // 실패해도(토큰 만료, 네트워크 등) 서버 자체는 계속 돈다 - 조용히 로그만 남긴다.
  const refreshInsightsAndPublish = (label) =>
    threadsInsights
      .refreshAll({})
      .then(() => publishInsightsSummaryToGithub())
      .catch((err) => console.error(`[insights] ${label} 실패:`, err.message));
  setTimeout(() => refreshInsightsAndPublish("초기 갱신"), 5000);
  setInterval(() => refreshInsightsAndPublish("주기 갱신"), 60 * 60 * 1000);
});
