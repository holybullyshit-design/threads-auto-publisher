# 팔자명가 스레드 + Instagram 자동화

Threads 초안·예약 기능을 보존하면서 Instagram `@saju_orbit`용 오늘의 운세 7장 캐러셀 생성·검수·예약을 통합한 로컬 웹앱입니다.

## Instagram 오늘의 운세

- **2026-08-26부터 시작**하며, 오늘 06:00이 지났으면 다음 클라우드 실행에서 즉시 게시 대상이 됩니다.
- `lunar-javascript` 기반 연주·월주·일주를 계산하고 공개 검증 기준일 테스트를 수행합니다.
- 1~5장은 날짜별로 바뀌는 12띠·각 4개 성인 생년 운세, 6장은 `人間萬事塞翁之馬 / 인간만사 새옹지마`, 7장은 `누적 상담 7,000건+ / 상담 후기 2,100개+` 고정 카드입니다.
- Instagram은 매일 **06:00 KST**로 저장하며 Threads용 ±15분 오차를 적용하지 않습니다.
- 날짜별 `dedupeKey` + 캡션 고유 마커로 재시도 중복 게시를 방지합니다.
- 7장/이미지 크기/12띠/48개 문구/생년-띠/날짜-만세력/캡션/댓글 CTA 중 하나라도 틀리면 예약을 차단합니다.

Instagram 탭에서 `미리보기 → 전체 기간 검수 → 계정 사전 점검 → 일괄 예약` 순서로 진행합니다. 실제 게시를 위해 Instagram 전문 계정과 `.env`/GitHub Secrets의 `INSTAGRAM_USER_ID`, `INSTAGRAM_ACCESS_TOKEN`이 필요합니다.
...
`test:render`는 실행 컴퓨터의 Chrome으로 1080×1350 JPEG 7장을 실제 생성합니다.

Mac에서는 `팔자명가 자동화.command`를 더블클릭하면 앱이 실행되고 브라우저가 자동으로 열립니다.
